...
ListController.prototype.bind = function() {
	var classSelf = this;
	$('.tabs-frame').delegate('.icon-gouwuchekong', 'click', function(event) {
...
ListController.prototype.getProductList = function() {
	...
	if (pageIndex < pageTotalCount) {
		...
		this.request(classSelf.cardListApiUrl, requestData, {
			showLoadingTips: false,
			process: function(data) {
...
ListController.prototype.genProdcutItem = function(data) {
		...
		if (data != undefined) {
			htmlTpl = '<a href="/Card/Detail?Id=' + data.Id + '">';
			...
			htmlTpl += '<dd>本月已有 <span class="arial text-danger">' + data.Quantity + '</span> 人购买<i class="iconfont icon-gouwuchekong" data-id="' + data.Id + '"></i></dd>';
