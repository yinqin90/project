...
显示更多
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
OverseasSuitDetailController.prototype.swapTab = function () {
    $(".tabs-handle li").click(function () {
        var _this = $(this), index = _this.index();
        _this.addClass("on").siblings("li").removeClass("on");
        $("#tabCon > div").eq(index).show().siblings("div").hide();
    })
}
/*-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
弹出框电话号码样式
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
OverseasSuitDetailController.prototype.showPhoneModual = function () {
    var _this = this;
    if ($("#btnReserve").size === 0) return;
    require([this.utilStaticPrefix + "/url/url.min.js"], function () {
        var urlParam = $.url('?');
        $("#btnReserve").click(function () {
            ...
            $('#phoneNum').on('loaded.bs.modal', function (e) {
                $("#confirmtel").click(function () {
                    var storename = urlParam.storename,
                        productname=$.trim($("h3").text()),
                        orderdetail = $("#storetype").val() + " : " + storename+"---"+productname,
                        name = '手机用户',
                        tel = $('#tel').val(),
